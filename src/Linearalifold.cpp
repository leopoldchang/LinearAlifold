            ...
            if (smart_gap[j] - smart_gap[j-1] > smart_gap_threshold){
                ...
                while (1){
                    ...
                    if (next_position[j][jnext] != 0){
                        ...
                        if (jnext_min != 1000000000) jnext = jnext_min;
                        else jnext = -1;
                    ...
                    for (int p = i-1; ((smart_i -  smart_gap[p]) <= 2*SINGLE_MAX_LEN) and p >= 0; --p) {
                        ...
                        while (1){
                            ...
                            if (next_position[p][q] != 0){
                                ...
                                if (jnext_min != 1000000000) q = jnext_min;
                                else q = -1;
...
int main(int argc, char** argv){
    ...
    float pscore_for_this_structure = 0.;
    for (auto & pair : result_pairs){
        pscore_for_this_structure += pscore[pair.first][pair.second].ribo_score;
    }
    pscore_for_this_structure = -pscore_for_this_structure/n_seq/100.;

    printf("\n%s (%.2f = %.2f + %.2f)\n", result_alifold.structure.c_str(), printscore/n_seq, printscore/n_seq - pscore_for_this_structure, pscore_for_this_structure);
    if (is_verbose){
        printf("beam size %d\n", beamsize);
