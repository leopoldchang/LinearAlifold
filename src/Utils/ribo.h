...
int
vrna_hamming_distance(const string  s1,
                      const string  s2)
{
  int h = 0;
...
float **
get_ribosum(const vector<string> &Alseq,
            int         n_seq,
            int         length)
{
  int   i, j, k;
...
tuple<vector<vector<int>>, vector<vector<int>>, vector<vector<int>>, vector<vector<int>>> a2s_prepare_is(vector<std::string> &MSA, int n_seq, int MSA_seq_length, vector<vector<int>> &a2s, vector<vector<int>> &s5, vector<vector<int>> &s3, vector<vector<int>> &SS, vector<float> & smart_gap){

  a2s.resize(MSA_seq_length); 
  s5.resize(MSA_seq_length); 
  s3.resize(MSA_seq_length);
  SS.resize(MSA_seq_length); 
  smart_gap.resize(MSA_seq_length);

  for (int i = 0 ; i< MSA_seq_length; i++){
      a2s[i].resize(n_seq);
      s5[i].resize(n_seq);
      s3[i].resize(n_seq);
      SS[i].resize(n_seq);
  ...
  for (int s = 0; s < n_seq; s++){
    ...
    s5[0][s]= 4; //4 is N in our system, not in RNAfold (which N = 0)
    s3[MSA_seq_length - 1][s] = 4;

    if (MSA[s][0] == '-') a2s[0][s] = -1;
    else a2s[0][s] = 0;

    for (int ss = 0; ss < MSA_seq_length; ss++) {

      if (MSA[s][ss] == '-'){
        SS[ss][s] = 5;
      }
      else{
        auto MSA_s_ss_num = GET_ACGU_NUM(MSA[s][ss]);

        SS[ss][s] = MSA_s_ss_num;
      }
    }


    for (int ss = 0; ss < MSA_seq_length - 1; ss++) {
      ...
      if (MSA[s][ss] == '-')
        ...
        s5[ss + 1][s] = MSA_s_ss_num;

    }

    for (int ss = MSA_seq_length - 1; ss > 0; ss--) {
      auto MSA_s_ss_num = GET_ACGU_NUM(MSA[s][ss]);
  ...
  for (int i = 0; i < MSA_seq_length ; i ++){
    ...
    perc = float(count)/n_seq;

    if (i == 0){
      smart_gap[i] = perc;
    } 
    else{
      smart_gap[i] = perc + smart_gap[i-1];
    }
  }

  return make_tuple(a2s, s5, s3, SS);
...
int AUCG_pair_type(char & l, char & r){
    // cout<<int('A')<<' '<<int('U')<<' '<<int('C')<<' '<<int('G')<<endl;
    //           65              85             67            71

    if (l == 67 and r == 71){     //CG
            return 1;
    }
    else if (l == 71 and r == 67){   //GC
        return 2;
    }
    else if (l == 71 and r == 85){   //GU
        return 3;
    }    
    else if (l == 85 and r == 71){  //UG
        return 4;
    }
    else if (l == 65 and r == 85){  //AU
        return 5;
    }
    else if (l == 85 and r == 65){  //UA
        return 6;
    }
...
vector<vector<ribo_state>> init_pscores_only(int MSA_seq_length){
    vector<vector<ribo_state>> pscore;

    pscore.resize(MSA_seq_length);

    for (int i = 0; i< MSA_seq_length; i++){
        pscore[i].resize(MSA_seq_length);
    }
    return pscore;
}
...
vector<vector<int>> init_next_position_only(int MSA_seq_length){
    vector<vector<int>> next_position;

    next_position.resize(MSA_seq_length);

    for (int i = 0; i< MSA_seq_length; i++){
        next_position[i].resize(MSA_seq_length);
    }
...
// ACGU
// 0123 //from contrafold, but used here
...
// type CONTRAfold nuc indices, not Vienna's
// CG 12  1
// GC 21  2
...
// other 0
int get_AUCG_pair_type_CONTRAfold_indices(int l, int r){

  if (l == 1 and r == 2){     //CG
      return 1;
  }
  else if (l == 2 and r == 1){   //GC
      return 2;
  }
  else if (l == 2 and r == 3){   //GU
      return 3;
  }    
  else if (l == 3 and r == 2){  //UG
      return 4;
  }
  else if (l == 0 and r == 3){  //AU
      return 5;
  }
  else if (l == 3 and r == 0){  //UA
      return 6;
  }
...
int make_pscores_ij(vector<int> & SS_fast_i, vector<int> & SS_fast_j, float ** ribo){

    auto n_seq = SS_fast_i.size();
    ...
    for (int s = 0; s < n_seq; s++){
        if (SS_fast_i[s] == 5 && SS_fast_j[s] == 5){ 
            type = 7;
        }
        else{
            type = get_AUCG_pair_type_CONTRAfold_indices(SS_fast_i[s], SS_fast_j[s]);
        }
